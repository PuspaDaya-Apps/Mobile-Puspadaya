...
class JadwalPosyanduHomeBloc extends Bloc<JadwalPosyanduHomeEvent, JadwalPosyanduHomeState> {
  ...
  Future<void> getJadwalHome (GetJadwalHome event, Emitter<JadwalPosyanduHomeState> emit) async {
    ...
    if(accessToken == null) {
      ...
      try {
        ...
        if(statusCode == 200) {
          List<Data> dataTemp = cardHomeResponseModel.data!.where((value) 
            => value.tanggalPelaksanaan.isAfter(DateTime.now()) ||
            value.tanggalPelaksanaan.isAtSameMomentAs(DateTime.now())
          ).toList();

          dataTemp.sort((a, b) => a.tanggalPelaksanaan.compareTo (b.tanggalPelaksanaan));
